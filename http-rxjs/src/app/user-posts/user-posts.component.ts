...
export class UserPostsComponent implements OnInit {
  ...
  ngOnInit(): void {
    forkJoin({
      user1: this.userService.getRandomUser().pipe(delay(2000)),
      user2: this.userService.getRandomUser().pipe(delay(3000)),
      user3: this.userService.getRandomUser().pipe(delay(6000)),
  ...
  getUserNPostCountWithE(id: number, posts: Posts[]): number {
