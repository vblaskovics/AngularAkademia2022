...
import { interval, Observable, Subject, map, count, filter } from 'rxjs';

export class StateService {

  clock: Observable<number> = interval(1000);

    counter$: Observable<number> //counter stream $-lel
  ...
  constructor() {
    ...
    this.counter$ = this.clock.pipe(map((count)=> {
      return this.counterDeadline - count;
    }))

    //hogy ha új érték lesz ezen a streamen, akkor futtauk le rajt a filtert
